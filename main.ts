...
serve(async (req) => {
    ...
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6">
      <div class="max-w-[1050px] mx-auto text-center">
        ...
        <section class="mb-12 px-10">
          ...
          <p class="text-zinc-500 text-lg leading-relaxed italic max-w-3xl mx-auto">
            Expert analysis with high-accuracy predictions. Match names and tips are locked for security.
          </p>
        </section>
        ...
        <div id="plans-ui" class="grid grid-cols-2 gap-10 mb-20 max-w-4xl mx-auto text-center">
            <div class="card-bg rounded-2xl p-10 border-b-4 border-yellow-600 transition hover:-translate-y-1">
                ...
                <div class="bg-sky-600 py-3 rounded-full font-black text-sm uppercase cursor-not-allowed opacity-50">Buy via Admin</div>
            </div>
        ...
        <div id="dashboard-header" class="hidden">
           <div class="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800 mb-8 flex justify-between items-center text-left">
              <div>
                ...
                <p class="text-zinc-500 text-sm font-bold mt-1 uppercase">Available Credits: <span id="displayCredits" class="bg-sky-900 text-sky-400 px-3 py-0.5 rounded ml-2">0</span></p>
              </div>
      ...
      <script>
        ...
        if(isLoggedIn) {
          ...
          else if(now > userData.expiry) { alert('Expired!'); logout(); }
          else {
        ...
        async function fetchTips() {
          ...
          document.getElementById('tips-table-body').innerHTML = data.map(t => {
            ...
            let matchText = isUnlocked ? t.match : '<span class="text-zinc-700 tracking-[0.2em] font-black uppercase">Locked Info</span>';
            ...
            let oddsText = isUnlocked ? t.odds : '-';
            ...
            return '<tr class="' + rowClass + '">' +
              ...
              '<td class="p-4 text-yellow-500 font-bold text-lg border-r border-white/5">' + matchText + (t.isVip ? '<span class="vip-badge">VIP</span>' : '') + '</td>' +
              '<td class="p-4 font-black border-r border-white/5">' + tipText + '</td>' +
              '<td class="p-4 text-zinc-500 font-mono border-r border-white/5">' + oddsText + '</td>' +
        ...
        async function unlockTip(tipId) {
          if(!confirm('Use 1 credit to reveal this match and tip?')) return;
  ...
  if (url.pathname === "/admin" && req.method === "GET") {
    let adminContent = "";
    if (!storedPass) {
      adminContent = '<div class="card-bg p-8 rounded-xl shadow-2xl"><input type="password" id="newPass" placeholder="Set Admin Password" class="stripe-input"><button onclick="setPass()" class="btn-main">INITIAL SETUP</button></div>' +
      ...
      adminContent = '<div class="card-bg p-8 rounded-2xl mb-12 shadow-2xl border-t-4 border-sky-500">' +
                     '<h3 class="text-sky-500 font-black mb-6 uppercase text-xs">Add Credits to User</h3>' +
                     '<input type="password" id="adminKey" placeholder="Admin Secret Key" class="stripe-input">' +
                     '<div class="grid grid-cols-2 gap-4"><input type="text" id="targetUser" placeholder="Username" class="stripe-input"><input type="number" id="targetCredits" placeholder="Credits" class="stripe-input"></div>' +
                     '<div class="grid grid-cols-2 gap-4"><input type="text" id="targetPass" placeholder="User Password" class="stripe-input"><input type="date" id="expDate" class="stripe-input"></div>' +
                     '<button id="createBtn" class="bg-sky-600 w-full py-4 rounded font-bold uppercase text-xs">Update Member Info</button></div>' +
                     '<div id="user-list" class="space-y-2 mb-12"></div>' +
                     '<div class="card-bg p-8 rounded-2xl shadow-2xl border-t-4 border-yellow-500">' +
                     '<h3 class="text-yellow-500 font-black mb-6 uppercase text-xs">Post Over Tip</h3>' +
                     '<input type="text" id="date" placeholder="Date" class="stripe-input">' +
                     '<input type="text" id="match" placeholder="Match (e.g. Chelsea vs Arsenal)" class="stripe-input">' +
                     '<input type="text" id="tip" placeholder="Over Line (e.g. Over 2.5)" class="stripe-input">' +
                     ...
                     '<button id="saveBtn" class="bg-yellow-600 w-full py-4 rounded font-bold uppercase tracking-widest">Save Record</button></div>' +
                     '<script>' +
    ...
    const adminHtml = `<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6 max-w-2xl mx-auto"><h2 class="text-3xl font-black text-yellow-500 mb-8 italic uppercase text-center">Admin Console</h2>${adminContent}</body></html>`;
