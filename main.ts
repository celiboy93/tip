...
serve(async (req) => {
        ...
        <div id="guest-ui">
          ...
          <div class="grid grid-cols-2 gap-10 mb-20 max-w-4xl mx-auto">
              <div class="card-bg rounded-2xl p-10 border-b-4 border-yellow-600 shadow-2xl">
                  <h2 class="text-6xl font-black mb-4">$30</h2><p class="text-zinc-600 text-xs font-black uppercase tracking-widest">Standard Access Plan</p>
              </div>
              <div class="card-bg rounded-2xl p-10 border-b-4 border-sky-600 shadow-2xl">
                  <h2 class="text-6xl font-black mb-4">$300</h2><p class="text-zinc-600 text-xs font-black uppercase tracking-widest">VIP Intelligence Access</p>
              </div>
          </div>
        </div>
      ...
      <script>
        ...
        if(userData){
          if(userData.remUntil && Date.now()>userData.remUntil){ logout(); }
            ...
            isLoggedIn=true; document.getElementById('guest-ui').classList.add('hidden');
        ...
        async function fetchTips(){
          ...
          document.getElementById('tips-table-body').innerHTML=data.map(t=>{
            ...
            let tTxt=isUnlocked?('<span class="text-white font-bold">'+t.tip+'</span>'):
            ...
            return '<tr class="match-row">'+
              ...
              '<td class="p-4 text-yellow-500 font-bold text-lg border-r border-white/5">'+mTxt+'</td>'+
              '<td class="p-4 border-r border-white/5">'+tTxt+'</td>'+
              '<td class="p-4 text-zinc-500 font-mono border-r border-white/5">'+(isUnlocked?t.odds:'-')+'</td>'+
              '<td class="p-4 font-black text-2xl text-zinc-300 border-r border-white/5 text-center">'+(t.result||'-:-')+'</td>'+
        ...
        async function unlockTip(id){ askConfirm('Unlock this match for 1 credit?',async()=>{
  ...
  // 2. ADMIN PANEL (WITH SESSION LOGIN)
  if (url.pathname === "/admin" && req.method === "GET") {
    let adminUI = "";
    if (!storedPass) {
       adminUI = '<div class="card-bg p-8 rounded-xl shadow-2xl"><input type="password" id="newPass" class="stripe-input" placeholder="Initial Admin Password"><button onclick="setPass()" class="btn-main">SETUP</button></div>' +
       ...
       adminUI = `
        <div id="admin-login-box" class="card-bg p-8 rounded-xl shadow-2xl max-w-sm mx-auto">
           ...
           <button onclick="adminLogin()" class="btn-main">LOGIN TO DASHBOARD</button>
        </div>

        <div id="admin-dashboard" class="hidden">
          <div class="flex justify-between items-center mb-8 bg-zinc-900 p-4 rounded-lg">
             <span class="font-black text-yellow-500 uppercase tracking-widest text-xs">Admin Session Active</span>
             <button onclick="sessionStorage.removeItem('admin_key'); location.reload();" class="text-zinc-500 underline text-xs">Logout Admin</button>
          </div>
        ...
        <script>
          ...
          async function adminLogin() {
            ...
            const res = await fetch('/api/admin-verify', { method: 'POST', body: JSON.stringify({ pass }) });
            if(res.ok) { sessionStorage.setItem('admin_key', pass); location.reload(); } else { alert('Wrong Admin Key!'); }
          }

          async function saveUser() {
            const d = { adminKey: adminSessionKey, user: document.getElementById('targetUser').value, pass: document.getElementById('targetPass').value, credits: parseInt(document.getElementById('targetCredits').value) };
            const r = await fetch('/api/create-user', { method: 'POST', body: JSON.stringify(d) });
          ...
          async function loadAdminData() {
            ...
            document.getElementById('admin-tips').innerHTML = t.map(y => '<div class="card-bg p-3 flex justify-between items-center text-xs"><span>' + y.match + '</span><button onclick=\\'editT(' + JSON.stringify(y) + ')\\' class="text-sky-400 underline uppercase">Edit</button></div>').join('');
          }
  ...
  if (url.pathname === "/api/tips" && req.method === "GET") {
    ...
    tips.sort((a, b) => Number(b.id) - Number(a.id)); return new Response(JSON.stringify({ data: tips.slice(0, 15) }));
  }
