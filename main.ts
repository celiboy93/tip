...
serve(async (req) => {
  ...
  if (url.pathname === "/" && req.method === "GET") {
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6">
        ...
        <div class="card-bg rounded-2xl overflow-hidden shadow-2xl">
          <table class="w-full border-collapse">
            <thead><tr class="gold-gradient text-black text-[11px] font-black uppercase">
              <th class="p-4 border-r border-black/10">Date</th><th class="p-4 border-r border-black/10">Match Details</th><th class="p-4 border-r border-black/10">Over Line</th><th class="p-4 border-r border-black/10 text-center">Score</th><th class="p-4">Status</th>
            </tr></thead>
      ...
      <script>
        let currentPage = 1; const pageSize = 20; // ညီကိုလိုချင်သော ပွဲ ၂၀ ကန့်သတ်ချက်
        ...
        async function fetchTips(page = 1){
          currentPage = page;
          const res=await fetch('/api/tips?page=' + page + '&limit=' + pageSize); 
          const { data, totalPages } = await res.json(); // API မှ Paginated Data ရယူခြင်း
          ...
          for(let i=1; i<=totalPages; i++) {
             pgHtml += '<button onclick="fetchTips(' + i + ')" class="page-btn ' + (i === page ? 'active' : '') + '">' + i + '</button>';
          }
  ...
  if (url.pathname === "/api/tips" && req.method === "GET") {
    ...
    const paginatedTips = tips.slice(startIndex, startIndex + limit);
    return new Response(JSON.stringify({ data: paginatedTips, totalPages: Math.ceil(tips.length / limit) }));
  }

  // (All other APIs: Unlock, Create User, Login, Admin History, Change Pass remain same and functional)
  ...
  if (url.pathname === "/api/unlock-tip" && req.method === "POST") { const { user, pass, tipId } = await req.json(); const uE = await kv.get(["users", user]); const tE = await kv.get(["tips", tipId]); if (!uE.value || uE.value.pass !== pass) return new Response("Error", { status: 401 }); const tip = tE.value; if (tip.lockTime) { const now = new Date(); const [h, m] = tip.lockTime.split(':'); const lockDate = new Date(); lockDate.setHours(parseInt(h), parseInt(m), 0); if (now.getTime() > lockDate.getTime()) return new Response("Time Expired!", { status: 400 }); } const u = uE.value; if ((u.credits || 0) <= 0) return new Response("No Credits!", { status: 400 }); if (u.unlockedTips?.includes(tipId)) return new Response(JSON.stringify(u)); const updated = { ...u, credits: u.credits - 1, unlockedTips: [...(u.unlockedTips || []), tipId] }; await kv.set(["users", user], updated); await kv.set(["history", Date.now().toString()], { user, match: tip.match, time: new Date().toLocaleTimeString('en-GB') }); return new Response(JSON.stringify(updated)); }
