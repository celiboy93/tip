...
serve(async (req) => {
    ...
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6">
      ...
      <div class="max-w-[1050px] mx-auto text-center">
        ...
        <section class="mb-12 px-10">
          ...
          <p class="text-zinc-500 text-lg leading-relaxed italic max-w-3xl mx-auto">Expert analysis delivers high-accuracy predictions. Member access required for details.</p>
        </section>
        ...
        <div id="dashboard-header" class="hidden">
           <div class="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800 mb-8 flex justify-between items-center text-left">
              <div><h2 class="text-yellow-500 font-black text-2xl uppercase italic">Member: <span id="displayUser" class="text-white"></span></h2><p class="text-zinc-500 text-xs font-bold mt-1 uppercase">Credits Remaining: <span id="displayCredits" class="bg-sky-900 text-sky-400 px-3 py-0.5 rounded ml-1">0</span></p></div>
        ...
        <div class="card-bg rounded-2xl overflow-hidden shadow-2xl">
          <table class="w-full border-collapse">
            <thead><tr class="gold-gradient text-black text-[11px] font-black uppercase">
              ...
              <th class="p-4 text-center border-r border-black/10">Score</th><th class="p-4">Status</th>
            </tr></thead>
        ...
        <footer class="py-16 border-t border-zinc-900">
            <p class="text-zinc-500 text-[10px] font-black uppercase tracking-[0.4em]">&copy; 2025 WINNER-MATCH.DENO.DEV | ALL RIGHTS RESERVED</p>
        </footer>
      </div>
      <script>
  ...
  // 2. ADMIN PANEL (WITH VISIBLE CREDIT BALANCE)
       ...
       adminUI = `
        ...
        <div id="admin-dashboard" class="hidden">
          ...
          <div class="card-bg p-8 rounded-2xl mb-12 shadow-2xl border-t-4 border-sky-500">
            <h3 class="text-sky-500 font-black mb-4 uppercase text-xs">Credit Manager (Top-up)</h3>
            <div class="grid grid-cols-2 gap-4"><input type="text" id="targetUser" placeholder="User" class="stripe-input"><input type="number" id="targetCredits" placeholder="Credits to ADD" class="stripe-input"></div>
            <input type="text" id="targetPass" placeholder="User Password" class="stripe-input">
            <button onclick="saveUser()" class="bg-sky-600 w-full py-4 rounded font-bold text-xs uppercase tracking-widest">Add Credits</button>
          </div>

          <h3 class="text-zinc-500 font-black mb-4 uppercase text-[10px] tracking-widest">Active Member Credit Balances</h3>
        ...
        <script>
          ...
          async function loadAdminData() {
            ...
            // Credit လက်ကျန်ကို တောက်တောက်လွင်လွင် ပြသခြင်း
            document.getElementById('user-list').innerHTML = u.map(x => \`
                <div class="card-bg p-3 flex justify-between items-center text-xs border-l-4 border-sky-600">
                    <div>
                        <span class="font-bold text-white underline">\${x.user}</span>
                        <span class="ml-4 bg-sky-900/50 text-sky-400 px-3 py-0.5 rounded-full font-black">Credits: \${x.credits||0}</span>
                    </div>
          ...
          window.deleteU = async (u) => { if(!confirm('Delete User?')) return; await fetch('/api/delete-user', { method: 'POST', body: JSON.stringify({ adminKey: adminSessionKey, user: u }) }); location.reload(); };
  ...
  if (url.pathname === "/api/create-user" && req.method === "POST") {
    ...
    // Credit ပေါင်းထည့်ခြင်း Logic
    await kv.set(["users", user], { ...oldData, user, pass, credits: (oldData.credits || 0) + (credits || 0) });
