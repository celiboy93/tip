...
serve(async (req) => {
        ...
        <div id="dashboard-ui" class="hidden">
           <div class="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800 mb-8 flex justify-between items-center text-left">
              <div>
                ...
                <p class="text-zinc-500 text-sm font-bold mt-1">Status: <span class="text-green-500 uppercase">Active</span> | <span class="text-red-500 uppercase">Expiry: <span id="displayExp"></span></span></p>
              </div>
           ...
           <div class="card-bg rounded-2xl overflow-hidden shadow-2xl">
            <table class="w-full border-collapse">
              <thead><tr class="gold-gradient text-black text-[11px] font-black uppercase">
                <th class="p-4 border-r border-black/10 text-center">Date</th><th class="p-4 border-r border-black/10 text-center">Match</th>
                <th class="p-4 border-r border-black/10 text-center">Prediction</th><th class="p-4 border-r border-black/10 text-center">Odds</th><th class="p-4 border-r border-black/10 text-center">Score</th><th class="p-4 text-center">Status</th>
              </tr></thead>
  ...
  if (url.pathname === "/admin" && req.method === "GET") {
    ...
    if (!storedPass) {
      content = `
        <div class="card-bg p-8 rounded-xl shadow-2xl">
          <input type="password" id="newPass" placeholder="Set Admin Password" class="stripe-input">
          <button onclick="setPass()" class="btn-main">INITIAL SETUP</button>
        </div>
        <script>
      ...
      content = `
        <div class="card-bg p-8 rounded-2xl mb-12 shadow-2xl border-t-4 border-sky-500">
          <h3 class="text-sky-500 font-black mb-6 uppercase tracking-widest">Add New Member</h3>
          <input type="password" id="adminKey" placeholder="Admin Secret Key" class="stripe-input">
          <div class="grid grid-cols-2 gap-4">
            <input type="text" id="targetUser" placeholder="Username" class="stripe-input">
            <input type="text" id="targetPass" placeholder="Password" class="stripe-input">
          </div>
          <input type="date" id="expDate" class="stripe-input">
          <button id="createBtn" class="bg-sky-600 w-full py-4 rounded-lg font-black uppercase text-sm tracking-widest">Create Account</button>
        </div>
        ...
        <script>
          async function loadAdminData() {
            ...
            document.getElementById('user-list').innerHTML = users.map(u => 
              '<div class="card-bg p-4 flex justify-between items-center text-xs"><span>👤 ' + u.user + ' (Exp: ' + new Date(u.expiry).toLocaleDateString() + ')</span><button onclick=\\'deleteUser("' + u.user + '")\\' class="text-red-500 font-black underline">DEL</button></div>'
            ).join('');
          }
    ...
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6 max-w-2xl mx-auto"><h2 class="text-3xl font-black text-yellow-500 mb-8 italic uppercase tracking-tighter">Admin Dashboard</h2>${content}</body></html>`, { headers: { "Content-Type": "text/html; charset=UTF-8" } });
  }
  ...
  if (url.pathname === "/api/user-login" && req.method === "POST") {
    ...
    } else { return new Response("Error", { status: 401 }); }
  }
