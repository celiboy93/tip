...
serve(async (req) => {
    ...
    if (!storedPass) {
       ...
       adminUI = `
        ...
        <div id="admin-dashboard" class="hidden">
          ...
          <div class="card-bg p-6 rounded-xl mb-12 border-t-4 border-green-600 shadow-2xl"><h3 class="text-green-500 font-black mb-4 uppercase text-xs">Live Unlock History</h3><div id="history-list" class="space-y-2 max-h-[300px] overflow-y-auto pr-2"></div></div>
          ...
          <div class="card-bg p-8 rounded-2xl border-t-4 border-yellow-500 mb-10"><h3 class="text-yellow-500 font-black mb-4 uppercase text-xs">Post Tip with Time Lock</h3><input type="text" id="tipId" placeholder="ID (Auto for New)" class="stripe-input"><div class="grid grid-cols-2 gap-4"><input type="text" id="date" placeholder="Date" class="stripe-input"><input type="time" id="lockTime" class="stripe-input"></div><input type="text" id="match" placeholder="Match Details" class="stripe-input"><input type="text" id="tip" placeholder="Over Line" class="stripe-input"><div class="grid grid-cols-2 gap-4"><input type="text" id="odds" placeholder="Odds" class="stripe-input"><input type="text" id="result" placeholder="Score" class="stripe-input"></div><select id="status" class="stripe-input !bg-zinc-900"><option value="Pending">Pending</option><option value="Win">Win</option><option value="Draw">Draw</option><option value="Lose">Lose</option></select><button onclick="saveTip()" class="bg-yellow-600 text-black w-full py-4 rounded font-black uppercase">SAVE TIP</button></div>
        ...
        <script>
          ...
          async function saveUser() { const d = { adminKey: adminSessionKey, user: document.getElementById('targetUser').value, pass: document.getElementById('targetPass').value, credits: parseInt(document.getElementById('targetCredits').value || 0) }; const r = await fetch('/api/create-user', { method: 'POST', body: JSON.stringify(d) }); location.reload(); }
  ...
  // (Unlock, Create User, Login, History, Change Pass remain same and functional)
  if (url.pathname === "/api/unlock-tip" && req.method === "POST") {
    ...
    const u = uE.value; if ((u.credits || 0) <= 0) return new Response("No Credits!", { status: 400 });
