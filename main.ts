...
serve(async (req) => {
    ...
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6">
      ...
      <div class="max-w-[1050px] mx-auto text-center">
        ...
        <section class="mb-12 px-10 text-center">
          ...
          <p class="text-zinc-500 text-lg leading-relaxed italic max-w-3xl mx-auto">Expert analysis powered by statistical data for high-accuracy predictions. Elevate your winning game today.</p>
        ...
        <div id="guest-ui">
          ...
          <div class="grid grid-cols-2 gap-10 mb-20 max-w-4xl mx-auto">
              <div class="card-bg rounded-2xl p-10 border-b-4 border-yellow-600 shadow-2xl transition hover:-translate-y-1"><h2 class="text-6xl font-black mb-4">$70</h2><p class="text-zinc-400 text-xs font-black uppercase tracking-widest italic">7 Tips Package</p></div>
              <div class="card-bg rounded-2xl p-10 border-b-4 border-sky-600 shadow-2xl transition hover:-translate-y-1"><h2 class="text-6xl font-black mb-4">$250</h2><p class="text-zinc-400 text-xs font-black uppercase tracking-widest italic">30 Tips VIP Bundle</p></div>
          </div>
        </div>

        <div id="dashboard-header" class="hidden">
           <div class="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800 mb-8 flex justify-between items-center text-left shadow-xl">
      ...
      <script>
        ...
        async function fetchTips(){
          ...
          document.getElementById('tips-table-body').innerHTML=data.map(t=>{
            ...
            // Date Visibility Fix - Zinc-200 and Font-Black
            return '<tr class="match-row"><td class="p-4 text-zinc-200 text-sm font-black border-r border-white/5">'+t.date+'</td><td class="p-4 text-yellow-500 font-bold text-lg border-r border-white/5">'+mTxt+'</td><td class="p-4 border-r border-white/5">'+tTxt+'</td><td class="p-4 text-zinc-500 font-mono border-r border-white/5">'+(isUnlocked?t.odds:'-')+'</td><td class="p-4 font-black text-2xl text-zinc-300 border-r border-white/5 text-center">'+(t.result||'-:-')+'</td><td class="p-4 '+sClass+' italic text-3xl uppercase tracking-tighter">'+t.status+'</td></tr>';
          }).join('');
        }
    ...
    if (!storedPass) {
       ...
       adminUI = `
        ...
        <div id="admin-dashboard" class="hidden">
          ...
          <div class="card-bg p-8 rounded-2xl mb-12 border-t-4 border-sky-500"><h3 class="text-sky-500 font-black mb-4 uppercase text-xs">Credit Top-up</h3><div class="grid grid-cols-2 gap-4"><input type="text" id="targetUser" placeholder="User" class="stripe-input"><input type="number" id="targetCredits" placeholder="ADD Credits" class="stripe-input"></div><input type="text" id="targetPass" placeholder="User Pass (Empty to Keep)" class="stripe-input"><button onclick="saveUser()" class="bg-sky-600 w-full py-4 rounded font-bold text-xs uppercase tracking-widest">Update User</button></div>
  ...
  if (url.pathname === "/api/unlock-tip" && req.method === "POST") {
    ...
    await kv.set(["users", user], updated); await kv.set(["history", Date.now().toString()], { user, match: tip.match, time: new Date().toLocaleTimeString('en-GB') });
  ...
  if (url.pathname === "/api/tips" && req.method === "GET") { const iter = kv.list({ prefix: ["tips"] }); const tips = []; for await (const res of iter) tips.push(res.value); tips.sort((a, b) => Number(b.id) - Number(a.id)); return new Response(JSON.stringify({ data: tips.slice(0, 30) })); }
  ...
  if (url.pathname === "/api/create-user" && req.method === "POST") { const { adminKey, user, pass, credits } = await req.json(); if (adminKey !== storedPass) return new Response("Error", { status: 401 }); const ex = await kv.get(["users", user]); const old = ex.value || { credits: 0, unlockedTips: [] }; const finalPass = pass || old.pass; await kv.set(["users", user], { ...old, user, pass: finalPass, credits: (old.credits || 0) + (credits || 0) }); return new Response("OK"); }
