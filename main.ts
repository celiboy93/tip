...
serve(async (req) => {
  ...
  if (url.pathname === "/" && req.method === "GET") {
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6">
      ...
      <script>
        let currentPage = 1; function showToast(m, t='info'){ const c=document.getElementById('toast-container'); const el=document.createElement('div'); el.className='toast '+t; el.innerText=m; c.appendChild(el); setTimeout(()=>el.classList.add('show'),10); setTimeout(()=>{el.classList.remove('show'); setTimeout(()=>el.remove(),300)},3000); }
  ...
  // 2. ADMIN PANEL (FIXED DUPLICATE & DELETE LOGIC)
  if (url.pathname === "/admin" && req.method === "GET") {
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6 max-w-2xl mx-auto">
        <div id="toast-container"></div>
        <h2 class="text-3xl font-black text-yellow-500 mb-8 italic uppercase text-center tracking-tighter">Admin Console</h2>
        ${!storedPass ? \`<div class="card-bg p-8 rounded-xl"><input type="password" id="newPass" class="stripe-input" placeholder="Setup Password"><button onclick="setPass()" class="btn-main">SETUP</button></div><script>async function setPass(){ const pass=document.getElementById("newPass").value; await fetch("/api/config",{method:"POST",body:JSON.stringify({pass})}); location.reload(); }</script>\` : \`
        ...
        <div id="admin-dashboard" class="hidden">
          ...
          <div class="card-bg p-8 rounded-2xl mb-12 shadow-2xl border-t-4 border-sky-500">
            ...
            <input type="text" id="targetPass" placeholder="User Pass (Empty to Keep)" class="stripe-input"><button onclick="saveUser()" class="bg-sky-600 w-full py-4 rounded font-bold text-xs uppercase tracking-widest">Save User Info</button>
          </div>
          <div id="user-list" class="space-y-2 mb-12"></div>
          <div class="card-bg p-8 rounded-2xl border-t-4 border-yellow-500 mb-10" id="form-section">
            ...
            <button onclick="saveTip()" class="bg-yellow-600 text-black w-full py-4 rounded font-black uppercase">Save Record</button>
            <button onclick="location.reload()" class="w-full mt-4 text-zinc-600 uppercase text-[10px] font-bold">Cancel / Clear Form</button>
          </div>
          <h3 class="text-zinc-500 uppercase text-[10px] font-black mb-4 tracking-widest">Editable History (Latest 30)</h3>
        ...
        <script>
          ...
          async function saveTip() {
            ...
            if(r.ok) { showToast('✅ Success! Tip Saved.', 'success'); setTimeout(()=>location.reload(), 1000); } else { showToast('❌ Error Saving!', 'error'); }
          }
          ...
          window.deleteT = async (id) => { if(!confirm('Delete this Match Tip?')) return; await fetch('/api/delete-tip', { method: 'POST', body: JSON.stringify({ adminKey: adminSessionKey, id }) }); showToast('✅ Deleted!', 'success'); loadAdminData(); };
  ...
  // (Verify, Create User, Login APIs stay functional)
  ...
  if (url.pathname === "/api/unlock-tip" && req.method === "POST") {
    ...
    const updated = { ...u, credits: u.credits - 1, unlockedTips: [...(u.unlockedTips || []), tipId] }; await kv.set(["users", user], updated); return new Response(JSON.stringify(updated));
  }
