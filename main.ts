...
serve(async (req) => {
  ...
  if (url.pathname === "/" && req.method === "GET") {
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6">
      ...
      <div id="pass-modal" class="fixed inset-0 bg-black/90 hidden items-center justify-center z-[10001]">
         <div class="modal-content"><h3 class="text-md font-bold mb-6 text-yellow-500 uppercase italic">Change Password</h3><input type="password" id="oldPass" placeholder="Current Password" class="stripe-input"><input type="password" id="newPass" placeholder="New Password" class="stripe-input"><div class="flex gap-3"><button onclick="submitPassChange()" class="flex-1 bg-yellow-600 text-black font-black py-2 rounded-md text-xs">UPDATE</button><button onclick="document.getElementById('pass-modal').style.display='none'" class="flex-1 bg-zinc-800 text-zinc-400 font-bold py-2 rounded-md text-xs">CANCEL</button></div></div>
      </div>
        ...
        <div id="guest-ui">
          ...
          <div class="grid grid-cols-2 gap-10 mb-20 max-w-4xl mx-auto">
              <div class="card-bg rounded-2xl p-10 border-b-4 border-yellow-600 shadow-2xl">
                  <h2 class="text-6xl font-black mb-4">$70</h2><p class="text-zinc-400 text-xs font-black uppercase tracking-widest italic">7 Tips Package</p>
              </div>
              <div class="card-bg rounded-2xl p-10 border-b-4 border-sky-600 shadow-2xl">
                  <h2 class="text-6xl font-black mb-4">$250</h2><p class="text-zinc-400 text-xs font-black uppercase tracking-widest italic">30 Tips VIP Bundle</p>
              </div>
          </div>
        </div>
      ...
      <script>
        ...
        async function submitPassChange() {
          ...
          if(res.ok) { showToast('✅ Password Updated!', 'success'); setTimeout(()=>logout(), 1500); }
  ...
  if (url.pathname === "/admin" && req.method === "GET") {
    let adminUI = "";
    if (!storedPass) {
       adminUI = '<div class="card-bg p-8 rounded-xl shadow-2xl"><input type="password" id="newPass" class="stripe-input" placeholder="Setup Password"><button onclick="setPass()" class="btn-main">SETUP</button></div>' +
       ...
       adminUI = `
        ...
        <div id="admin-dashboard" class="hidden">
          ...
          <div class="card-bg p-6 rounded-xl mb-12 border-t-4 border-green-600 shadow-2xl">
             <h3 class="text-green-500 font-black mb-4 uppercase text-xs tracking-widest">Live Unlock History</h3>
             <div id="history-list" class="space-y-2 max-h-[300px] overflow-y-auto pr-2"></div>
          </div>

          <div class="card-bg p-8 rounded-2xl mb-12 border-t-4 border-sky-500">
            <h3 class="text-sky-500 font-black mb-4 uppercase text-xs">Credit Top-up</h3>
            <div class="grid grid-cols-2 gap-4"><input type="text" id="targetUser" placeholder="User" class="stripe-input"><input type="number" id="targetCredits" placeholder="ADD Credits" class="stripe-input"></div>
            <input type="text" id="targetPass" placeholder="User Pass (Empty to Keep)" class="stripe-input">
            <button onclick="saveUser()" class="bg-sky-600 w-full py-4 rounded font-bold text-xs uppercase tracking-widest">Update User</button>
          </div>
        ...
        <script>
          ...
          async function saveUser() { const d = { adminKey: adminSessionKey, user: document.getElementById('targetUser').value, pass: document.getElementById('targetPass').value, credits: parseInt(document.getElementById('targetCredits').value || 0) }; await fetch('/api/create-user', { method: 'POST', body: JSON.stringify(d) }); location.reload(); }
  ...
  if (url.pathname === "/api/admin-history" && req.method === "GET") {
    const iter = kv.list({ prefix: ["history"] }); const h = []; for await (const res of iter) h.push(res.value);
    return new Response(JSON.stringify(h.reverse().slice(0, 50)));
  }

  // (Login, Change Password, Tips APIs remain same)
  ...
  if (url.pathname === "/api/user-change-password" && req.method === "POST") {
    const { user, oldPass, newPass } = await req.json(); const e = await kv.get(["users", user]);
    if (!e.value || e.value.pass !== oldPass) return new Response("Current Password Incorrect!", { status: 401 });
    await kv.set(["users", user], { ...e.value, pass: newPass }); return new Response("OK");
  }
  ...
  if (url.pathname === "/api/create-user" && req.method === "POST") {
    ...
    const ex = await kv.get(["users", user]); const old = ex.value || { credits: 0, unlockedTips: [] };
    const finalPass = pass || old.pass; await kv.set(["users", user], { ...old, user, pass: finalPass, credits: (old.credits || 0) + (credits || 0) }); return new Response("OK");
  }
