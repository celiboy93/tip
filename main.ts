...
serve(async (req) => {
    ...
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6">
      ...
      <script>
        function showToast(m, t='info'){ 
          ...
          el.className='toast '+t; el.innerText=m; c.appendChild(el);
          setTimeout(()=>el.classList.add('show'),10); setTimeout(()=>{el.classList.remove('show'); setTimeout(()=>el.remove(),300)},3000); 
        }
        ...
        if(userData){ isLoggedIn=true; document.getElementById('guest-ui').classList.add('hidden'); document.getElementById('dashboard-header').classList.remove('hidden'); document.getElementById('displayUser').innerText=userData.user; document.getElementById('displayCredits').innerText=userData.credits||0; }
  ...
  // 2. ADMIN PANEL (WITH FIXED NOTI & EDIT LOGIC)
  if (url.pathname === "/admin" && req.method === "GET") {
    return new Response(`<!DOCTYPE html><html><head>${UI_HEAD}</head><body class="p-6 max-w-2xl mx-auto">
        ...
        <div id="admin-dashboard" class="hidden">
          ...
          <div class="card-bg p-8 rounded-2xl border-t-4 border-yellow-500 mb-10" id="edit-form">
